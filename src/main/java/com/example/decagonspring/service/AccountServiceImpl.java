...
public class AccountServiceImpl implements AccountService {
    ...
    @Override
    public synchronized String createAccount(String accountName, String phoneNo) {
    ...
    @Override
    public synchronized TransactionResponse deposit(String accountNumber, double amount) {
        ...
        return new TransactionResponse(amount, account.getBalance());
    }

    @Override
    public synchronized TransactionResponse withdraw(String accountNumber, double amount) {
        if (amount < 1) throw new ApiRequestException("invalid amount: amount cannot be lass than 1");
        if (accountNumber == null || accountNumber.trim().isEmpty() || accountNumber.length()!=10 || !accountMap.containsKey(accountNumber))
            throw new ApiRequestException("invalid account number");
        Account account = accountMap.get(accountNumber);
    ...
    public synchronized List<Transaction> getTransactions(String accountNumber) {
        if (accountNumber == null || accountNumber.trim().isEmpty()) {
            ...
            List<Transaction> transactions = transactionList.stream().filter(transactionsThatMatchAccountNumber).collect(Collectors.toList());
